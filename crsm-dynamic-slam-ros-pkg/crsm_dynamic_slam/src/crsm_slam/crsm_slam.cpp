...
namespace crsm_slam
{
  /**
    ...
    @return void
   **/
  CrsmSlam::CrsmSlam(int argc, char **argv): ObjectArea(0.24)
  {
    updateParameters();
  ...
  void CrsmSlam::fixNewScans(sensor_msgs::LaserScanConstPtr const &msg)
  {
    ...
    for(auto it = scanSelections.cbegin(); it != scanSelections.cend(); it++)
    {
  ...
  void CrsmSlam::calculateCriticalRays()
  {
    ...
    for(auto it = bigChanges.cbegin(); it != bigChanges.cend();)
    {
      double sumDensity = 0;
      start = *it;
      it++;
      
      if(it == bigChanges.cend())
				break;
  ...
  void CrsmSlam::findTransformation()
  {
    ...
    while(true)
    {
      ...
      for(auto j = scanSelections.cbegin(); j != scanSelections.cend(); j++)
      {
      ...
      if(trier.fitness > bestFitness)
      {
        ...
        trier.t.dth += ( rand() % slamParams.disparity - slamParams.disparity / 2.0 ) / 360.0;
      }
      else
      {
        ...
        trier.t.dth = (rand() % slamParams.disparity - slamParams.disparity / 2.0) / 180.0;
      }
  ...
  void CrsmSlam::updateMapProbabilities()
  {
      ...
      if(laser.scan.distance[measid] == 0 or laser.scan.distance[measid] == laser.info.laserMax)
        continue;
			...
			while(R < dMeasure + 1)
			{
				...
				if(R < dMeasure)
				{
					if(staticMap.p[xPoint][yPoint] < 255)
						staticMap.p[xPoint][yPoint]++; // +1 increment for safe maintenance of static parts
				...
				{
					int const br = ceil( staticMap.p[xPoint][yPoint] * (256 - staticMap.p[xPoint][yPoint]) / 1020 ); // Blackening rate
					// Denominator defines the reduction rate of occupancy probability
    ...
    k = dynamicObject.empty() ? 1 : dynamicObject.size();
  ...
  std::vector<CrsmPoint> CrsmSlam::findCentroids(std::vector<CrsmPoint> const &dynamicPoint)
  {
		...
		while(true)
		{
			...
			for(int i = 0; i < k; i++)
			{
				...
				else
				{
					...
					for(auto it = cluster[i].cbegin(); it != cluster[i].cend(); it++)
					{
						sumX += (*it).x;
						sumY += (*it).y;
					}
			...
			for(int i = 0; i < dynamicPoint.size(); i++)
			{
				...
				int const indexOfMin = std::distance( distanceFromCentroid, std::min_element(distanceFromCentroid, distanceFromCentroid + k) );
	...
	{
		for(auto it1 = currentCentroids.cbegin(); it1 != currentCentroids.cend() - 1; it1++)
		{
			for(auto it2 = it1 + 1; it2 != currentCentroids.cend(); it2++)
			{
	...
	void CrsmSlam::updateObjects(std::vector<CrsmPoint> *centroidsScanned, double const currentExecutionTime)
	{
		...
		for(auto it = dynamicObject.begin(); it != dynamicObject.end(); it++)
		{
			...
			auto best = centroidsScanned -> begin();
			double minDistance = Distance( (*it).getCentroid(), *best );
			
			for(auto it2 = centroidsScanned -> begin() + 1; it2 != centroidsScanned -> end(); it2++)
			{
		...
		for(auto it = dynamicObject.begin(); it != dynamicObject.end();)
		{
			if( (*it).timeUnscanned() > 10 * currentExecutionTime )
			{
				deleted.push_back( (*it).getID() );
				deleted.push_back( (*it).getID() + 1);
				
				dynamicObject.erase(it);
			}
			else
				it++;
		}
		...
		for(auto it = centroidsScanned -> begin(); it != centroidsScanned -> end(); it++)
		{	
			dynamicObject.push_back( CrsmDynamicObject (*it, id += 2) );
		}
	}
	...
	void CrsmSlam::publishMarkers()
	{
		...
		for(int i = 0; i < ( 2 * dynamicObject.size() ); i++)
		{
			visualization_msgs::Marker marker;
			...
			marker.ns = "marker_ns";
			marker.id = (i % 2 == 0) ? dynamicObject[i / 2].getID() : dynamicObject[i / 2].getID() + 1;
			
			marker.pose.position.x = dynamicObject[i / 2].getCentroid().x * slamParams.ocgd - staticMap.info.originx * slamParams.ocgd;
			marker.pose.position.y = dynamicObject[i / 2].getCentroid().y * slamParams.ocgd - staticMap.info.originy * slamParams.ocgd;
			marker.pose.position.z = 0.0;
			
			marker.pose.orientation.x = 0.0;
			marker.pose.orientation.y = 0.0;
			marker.pose.orientation.z = (i % 2 == 0) ? 0.0 : sin(dynamicObject[i / 2].getYawAngle() / 2);
			marker.pose.orientation.w = (i % 2 == 0) ? 1.0 : cos(dynamicObject[i / 2].getYawAngle() / 2);
			...
			else
			{
				...
				marker.scale.x = dynamicObject[i / 2].still() ? 0.0 : 0.5;
				marker.scale.y = dynamicObject[i / 2].still() ? 0.0 : 0.1;
				marker.scale.z = 0.0;
			}
			
			marker.action = visualization_msgs::Marker::ADD;
			
			marker.color.r = (float) (i % 2 == 0) * dynamicObject[i / 2].getR();
			marker.color.g = (float) (i % 2 == 0) ? dynamicObject[i / 2].getG() : 1;
			marker.color.b = (float) (i % 2 == 0) * dynamicObject[i / 2].getB();
			marker.color.a = 1.0f;
