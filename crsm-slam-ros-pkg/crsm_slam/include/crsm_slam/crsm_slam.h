...
namespace crsm_slam
{
  ...
  class CrsmSlam
  {
    private:
      ...
      ros::Publisher _dynamicOccupancyGridPublisher; //!< The occupancy grid map publisher
      ...
      int k; //!< Variable for use in kMeans() function
      int id; //!< Variable used for assigning different ids to dynamic objects
      int const Steps; //!< Steps used to calculate mean velocity of dynamic object
      double const ObjectArea; //!< Area [meters] that defines the minimum space occupied by a dynamic object
      
      std::vector<CrsmDynamicObject> object; //!< Vector that contains the dynamic objects scanned by the laser
    ...
    public:
      ...
      void updateMapProbabilities();
     	...
     	/**
				...
				@brief Draws free space around a dynamic point
				@param Central dynamic point
				@return void
			 **/
      void drawFreeSpace(CrsmPoint const &Point);
     	...
     	/**
				@author Efthimis Tsiogas, efthimis91@gmail.com
				@brief Checks if a map point is too close to a static point
				@param Point to be checked
				@return true if point is too close to a static one, false otherwise
			 **/
      bool closeToStatic(CrsmPoint const &Point);
      
			/**
				@author Efthimis Tsiogas, efthimis91@gmail.com
				@brief Calculates the Euclidean distance between two CrsmPoints
				@param First CrsmPoint
				@param Second CrsmPoint
				@return The distance between the two CrsmPoints
			 **/
      ...
      /**
        ...
        @param Dynamic points scanned by the laser
      ...
      /**
				@author Efthimis Tsiogas, efthimis91@gmail.com
        @brief Checks if any two centroids are too close to each other
        @param Centroids to check
        @return true if any two centroids are too close to each other, false otherwise
       **/
      ...
      /**
        @brief Starts the dynamic OccupancyGrid publisher, posting to dynamic_occupancy_grid_publish_topic from parameters
        @return void
       **/
      void startDOGMPublisher();
      
			/**
        @brief Publishes the dynamic OccupancyGrid map as nav_msgs::OccupancyGrid, posting with occupancy_grid_map_freq Hz from parameters
        @param e [const ros::TimerEvent&] The timer event
        @return void
       **/
      void publishDOGM(ros::Time timestamp);
