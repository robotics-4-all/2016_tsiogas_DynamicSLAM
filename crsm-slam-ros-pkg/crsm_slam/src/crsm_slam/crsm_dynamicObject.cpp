...
namespace crsm_slam
{
	...
	void CrsmDynamicObject::update(int const Steps, CrsmPoint const &newCentroid)
	{
		if(dx.size() == Steps)
		{
			dxSum -= dx.front();
		...
		// dt is zero only the very first time the program runs so floating point exception should be impossible
